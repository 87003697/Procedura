...
const descriptor: ToolDescriptor = { name: "mapping_facts", description: "Read bounded candidate-to-GT transport residuals at finest depth, including candidate part weights when the input provides them. Mixed part weights and neighboring transport can create spillover, so these records do not prove that a part moved; compute conclusions from coherent patterns.", owner: { kind: "core" }, inputSchema: { type: "object", additionalProperties: false, properties: { cursor: { type: "integer", minimum: 0 }, limit: { type: "integer", minimum: 1, maximum: 256 } } } satisfies JsonObject };
...
function candidatePartIndex(input: Record<string, unknown>): Map<number, JsonObject[]> {
  const index = new Map<number, JsonObject[]>();
  const candidate = input.candidate;
  if (!candidate || typeof candidate !== "object" || Array.isArray(candidate)) return index;
  const cells = (candidate as Record<string, unknown>).cells;
  if (!Array.isArray(cells)) return index;
  for (const value of cells) {
    if (!value || typeof value !== "object" || Array.isArray(value)) continue;
    const cell = value as Record<string, unknown>;
    if (typeof cell.prefix !== "number" || !Number.isInteger(cell.prefix) || !Array.isArray(cell.parts)) continue;
    index.set(cell.prefix, cell.parts.map((part) => {
      const item = record(part, "invalid candidate part");
      if (typeof item.name !== "string" || !item.name) throw Error("invalid candidate part name");
      return { name: item.name, weight: number(item.weight, "invalid candidate part weight") } as JsonObject;
    }));
  }
  return index;
}
...
export function makeMappingFactsTool(source: MappingFactsSource, maxCells = 20000): MappingFactsTool {
  ...
  const partsByPrefix = candidatePartIndex(input);
